...
class TestCourierLogin:
    ...
    def test_courier_login_successfully_logged_in(self, create_new_courier_and_return_created_courier_data, cleanup_created_courier):
        response = requests.post(LOGIN_COURIER_ENDPOINT_URL, json=create_new_courier_and_return_created_courier_data)
        assert response.status_code == 200
        assert 'id' in response.json()

        cleanup_created_courier.update(create_new_courier_and_return_created_courier_data)
